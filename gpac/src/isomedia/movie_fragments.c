...
/*DASH_IMPLEMENTATION_NIELSEN*/
#define ID3_TAG_SIZE 271
struct CPcmToId3Processor* PcmToID3;
extern char *id3_source;

int init_emsg(GF_DASHEventMessageBox **emsg, u64 prev_earliest_cts)
{
	int value=1;
	*emsg = (GF_DASHEventMessageBox *)gf_isom_box_new(GF_ISOM_BOX_TYPE_EMSG);

	if(*emsg)
	{
		if(((*emsg)->scheme_id_uri= gf_malloc(strlen("http://www.nielsen.com/id3/v1")+1)))
		{
			memset((*emsg)->scheme_id_uri, 0, strlen("http://www.nielsen.com/id3/v1")+1);
			strncpy((*emsg)->scheme_id_uri, "http://www.nielsen.com/id3/v1", strlen("http://www.nielsen.com/id3/v1"));
		}
		else
			return -1;

		if(((*emsg)->value = gf_malloc(sizeof(int)+1)))
		{
			memset((*emsg)->value, 0, sizeof(int)+1);
			snprintf((*emsg)->value,sizeof(int),"%d",value);
		}
		else
			return -1;

		(*emsg)->timescale = 90000;
		(*emsg)->presentation_time_delta = prev_earliest_cts;
...
int add_id3(GF_DASHEventMessageBox **emsg, uint8_t *pbuf, u64 id3_count)
{
	if(!id3_count)
	{
		if(((*emsg)->message_data = gf_malloc(ID3_TAG_SIZE+1)))
		{
			memset((*emsg)->message_data, 0, (ID3_TAG_SIZE)+1);
			memcpy((*emsg)->message_data, pbuf, ID3_TAG_SIZE);
			*((*emsg)->message_data+ID3_TAG_SIZE) = '\0';
			(*emsg)->id3_count=1;
		}
		else
			return -1;
	...
	{
		if(((*emsg)->message_data = gf_realloc((*emsg)->message_data, ((ID3_TAG_SIZE*(id3_count+1))+(id3_count+1)))))
		{
			*((*emsg)->message_data+((ID3_TAG_SIZE*id3_count)+(id3_count-1)))='\n';
			memcpy((*emsg)->message_data+(((ID3_TAG_SIZE*(id3_count))+id3_count)), pbuf, ID3_TAG_SIZE);
			*((*emsg)->message_data+(((ID3_TAG_SIZE*(id3_count+1))+id3_count)))='\0';
			(*emsg)->id3_count++;
		}
		else
			return -1;
...
GF_Err gf_isom_close_segment(GF_ISOFile *movie, s32 subsegments_per_sidx, u32 referenceTrackID, u64 ref_track_decode_time, s32 ts_shift, u64 ref_track_next_cts, Bool daisy_chain_sidx, Bool last_segment, u32 segment_marker_4cc, u64 *index_start_range, u64 *index_end_range)
{
	...
	GF_DASHEventMessageBox *emsg=NULL;/*DASH_IMPLEMENTATION_NIELSEN*/
	...
	GF_Err e;
	u64 emsg_end = 0;/*DASH_IMPLEMENTATION_NIELSEN*/
	...
	sidx_start = sidx_end = 0;
        ...
        if(i)
        {
                PcmToID3 = CInitPcmToId3(id3_source);
                i = 0;
        }
	...
	local_sidx_start = local_sidx_end = 0;
		...
		uint8_t *pbuf;
		int id3_count=0;
		u64 current_segment_start=0;
		u64 next_segment_start=0;
		current_segment_start = prev_earliest_cts/sidx->timescale;
		next_segment_start    = ref_track_next_cts/sidx->timescale;
		while (1)
		{
                	int seek_value=0;
			if (CGetQueue(PcmToID3, &pbuf) == 0)
			{
				seek_value = *((u_int *)(pbuf+4));
				if(!id3_count)
				{
					if((current_segment_start <= seek_value ) && (seek_value <= next_segment_start))
					{
						e = init_emsg(&emsg, prev_earliest_cts);
						if(e)
						{
							fprintf(stderr,"'emsg' box creation failed\n");
							break;
						}
						emsg->id = *((u_int *)(pbuf));
						add_id3(&emsg, pbuf, id3_count);
						if(e)
						{
							fprintf(stderr,"populating ID3 tags in 'emsg' box failed\n");
							break;
						}
						CRemoveQueue(PcmToID3);
						id3_count++;
					}
					else if(seek_value > next_segment_start)
					{
						break;
					}
					else if(seek_value < current_segment_start)
					{
						CRemoveQueue(PcmToID3);
					}
				}
				...
				{
					if(seek_value <= next_segment_start)
					{
						e = add_id3(&emsg, pbuf, id3_count);
						if(e)
						{
							fprintf(stderr,"populating ID3 tags in 'emsg' failed\n");
							break;
						}
						CRemoveQueue(PcmToID3);
						id3_count++;
			...
			{
				if(CIsEndReached(PcmToID3) == true)
				{
					break;
				}
				usleep(100000);
		...
		if(emsg != NULL)
		{
	                ...
	                e = gf_isom_box_size((GF_Box *) emsg);
        	        if (e) return e;
	                e = gf_isom_box_write((GF_Box *) emsg, movie->editFileMap->bs);
        	        if (e) return e;

	                gf_isom_box_del((GF_Box*)emsg);
        	        emsg_end = gf_bs_get_position(movie->editFileMap->bs);
                	last_top_box_pos = root_prev_offset = emsg_end;
		}
        /*DASH_IMPLEMENTATION_NIELSEN*/
        }
#endif
