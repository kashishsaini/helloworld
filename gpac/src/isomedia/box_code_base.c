...
{
        GF_DASHEventMessageBox *ptr = (GF_DASHEventMessageBox *) s;
        if (ptr == NULL) return;
...
GF_Box *emsg_New()
{
        ISOM_DECL_BOX_ALLOC(GF_DASHEventMessageBox, GF_ISOM_BOX_TYPE_EMSG);
        gf_isom_full_box_init((GF_Box *)tmp);
        return (GF_Box *)tmp;
}
/*DASH_IMPLEMENTATION_NIELSEN*/
...
GF_Err emsg_Write(GF_Box *s, GF_BitStream *bs)
{
        ...
        if (ptr->scheme_id_uri) {
                gf_bs_write_data(bs, ptr->scheme_id_uri, (u32)(strlen(ptr->scheme_id_uri)+1));
        }
        if (ptr->value) {
                gf_bs_write_data(bs, ptr->value, (u32)(strlen(ptr->value)+1));
        }
        ...
        gf_bs_write_u32(bs, ptr->id);
        if (ptr->message_data) {
		int loop = 0;
		for(;loop<ptr->id3_count;loop++)
		{
			gf_bs_write_data(bs, ptr->message_data+(272*loop), 272);
		}
...
GF_Err emsg_Size(GF_Box *s)
{
        ...
        if(ptr->scheme_id_uri) ptr->size += strlen(ptr->scheme_id_uri) + 1;
        if(ptr->value) ptr->size += strlen(ptr->value) + 1;
        ptr->size += 16;
        if(ptr->message_data) ptr->size += (272*(ptr->id3_count));
        return GF_OK;
}
/*DASH_IMPLEMENTATION_NIELSEN*/
