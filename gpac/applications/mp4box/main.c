...
void PrintDASHUsage()
{
	fprintf(stderr, "DASH Options:\n"
	        ...
	        " -id3-source          Original file with 'WaterMarks' for ID3 tag extraction (DASH_IMPLEMENTATION_NIELSEN)\n"
	        "\n");
}
...
char *inName, *outName, *arg, *mediaSource, *tmpdir, *input_ctx, *output_ctx, *drm_file, *avi2raw, *cprt, *chap_file, *pes_dump, *itunes_tags, *pack_file, *raw_cat, *seg_name, *dash_ctx_file, *id3_source/*DASH_IMPLEMENTATION_NIELSEN*/;
u32 track_dump_type;
u32 trackID;
...
Bool mp4box_parse_args(int argc, char **argv)
{
	...
	for (i = 1; i < (u32)argc; i++) {
		...
		else if (!stricmp(arg, "-id3-source")) {
			CHECK_NEXT_ARG id3_source = argv[i + 1];
			i++;
		}
...
int mp4boxMain(int argc, char **argv)
{
	...
	info_track_id = 0;
	do_flat = 0;
	inName = outName = mediaSource = input_ctx = output_ctx = drm_file = avi2raw = cprt = chap_file = pack_file = raw_cat = id3_source = NULL;/*DASH_IMPLEMENTATION_NIELSEN*/
	...
	if (dash_duration) {
		...
		if (!e) e = gf_dasher_enable_id3_source(dasher, id3_source);/*DASH_IMPLEMENTATION_NIELSEN*/
